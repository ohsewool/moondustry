...
**▶ 플레이: https://moondustry.osw1217.workers.dev**

- **물류가 핵심**: 드릴이 캔 구리를 컨베이어로 운반 → 코어에 넣으면 돈, 포탑에 꽂으면 탄약. 보급선이 끊기면 포탑은 침묵한다.
- **가공 사슬**: 티타늄+구리→**합금**(제련소), 모래+석탄→**실리콘**(실리콘로) — 팔거나 탄약으로
- **탄약이 무기**: 듀오에 실리콘탄=**유도탄**, 합금탄=고위력탄. 포탑은 장전된 탄약 종류를 기억하고, 교체는 소진 후에만
- **전력망**: 석탄→발전기(6⚡) + 노드 송전 — 랜서·스펙터·제련소·실리콘로는 전력이 부족하면 느려지고 없으면 멈춘다
- 라우터(분기), **정션**(컨베이어를 가로질러 놓으면 자동 생성되는 교차로), 멘더(수리), 벽, 포탑 4종(듀오/스캐터/랜서/스펙터) + **T1→T3 강화**
- 80×50 병목 지형 절차 생성 맵 + 휠 줌·WASD 팬 카메라, 적 5종(원거리 사격·자폭·공중·**5웨이브마다 보스**)
...
| 입력 | 동작 |
|---|---|
| 클릭 / 드래그 | 건설 (컨베이어는 드래그 방향으로 자동 회전, 기존 벨트를 가로지르면 정션) |
| `1`~`9`, `0` | 건물 선택 |
| 휠 / `WASD`·방향키 / 휠클릭 드래그 | 줌 / 카메라 이동 |
| `R` | 컨베이어 방향 회전 |
| 우클릭 | 판매 / 선택 취소 |
...
- `ITEM_VALUE`(코어 납품가: 구리 10 / 석탄 15 / 티타늄 25 / 실리콘 30 / 합금 60 / 모래 5) / `AMMO_PER`(구리 6 / 실리콘 8 / 합금 10발) / `AMMO_MAX`(30)
- `AMMO_TYPES` — 포탑별 사용 가능 탄약과 효과(유도·배율·산탄 추가) / `RECIPES` — 조합기 레시피(재료·산출·시간)
- `POWER_DRAW`(랜서 2 / 스펙터 4 / 제련소 2 / 실리콘로 2) / `GEN_OUT`(6⚡) / `COAL_BURN`(2초) / `NODE_LINK`·`NODE_COVER`
